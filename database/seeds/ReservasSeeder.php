...
class ReservasSeeder extends Seeder
{
    ...
    public function run()
    {
        $reservasBB = DB::select('SELECT * FROM sadebb.tb_oportunidade_reserva');
        ...
        foreach ($reservasBB as $reserva) {
            ...
            if (LicitacaoBB::find($reserva->id_bb_oportunidade))
                ReservaBB::firstOrCreate([
                    ...
                    'dt_inicio_upload' => $reserva->nm_tempo_inicio_upload,
                    'dt_fim_upload' => $reserva->nm_tempo_fim_upload,
                    'created_at' => $reserva->created_at,
                    'updated_at' => $reserva->updated_at,
                ]);
        }

        $reservasCN = DB::select('SELECT * FROM sadecn.reserva');
